# ✅ TODO EMJIJSYS

Roadmap ini dipakai untuk mengubah konsep menjadi paket gambar kerja yang lebih siap dicek.

Status terbaru: Rev3 memakai **1 KM shared + wastafel luar**, bukan 2 kamar mandi.

Catatan Antigravity V3: sebelum dikunci, buat studi **Rev 3.1** untuk membandingkan 1 KM proper, split bathroom, dan 2 KM compact.

## 🚨 Prioritas Tinggi

- [ ] Buat overlay grid resmi `1-3` dan `A-E`.
- [ ] Kodekan semua kolom, misalnya `K1-K15`.
- [ ] Buat denah balok lantai 2.
- [ ] Cek dinding lantai 2 terhadap posisi balok.
- [ ] Detailkan balok tepi void tangga.
- [ ] Detailkan balok balkon dan waterproofing balkon.
- [ ] Tentukan shaft plumbing untuk laundry + KM shared + wastafel luar.
- [ ] Buat studi alternatif Rev 3.1A: 1 KM shared proper + wastafel niche.
- [ ] Buat studi alternatif Rev 3.1B: split bathroom, kloset dan shower terpisah.
- [ ] Buat studi alternatif Rev 3.1C: 2 KM compact.
- [ ] Ubah wastafel luar menjadi niche/alcove anti-slip.
- [ ] Tambahkan pintu fisik/smart lock pada akses tangga dari lantai 1.
- [ ] Detail insulasi ruang kerja terhadap kamar utama.
- [ ] Pastikan koridor bersih minimal 900 mm.
- [ ] Simulasi swing semua pintu di area toilet, wastafel luar, ruang kerja, kamar utama, kamar tidur.

## 🏠 Arsitektur

- [ ] Finalkan foyer/arrival dekat tangga.
- [ ] Finalkan ruang tamu dekat tangga.
- [ ] Finalkan KM shared dan wastafel luar rev3.
- [ ] Finalkan dapur kering + meja makan.
- [ ] Kunci mushola 3000 x 2200 mm.
- [ ] Kunci balkon 3000 x 1500 mm.
- [ ] Atur furniture ruang keluarga.
- [ ] Atur posisi ranjang kamar tidur utama.
- [ ] Atur layout ruang kerja untuk rak dan soldering tools.
- [ ] Buat tampak depan.
- [ ] Buat potongan melalui tangga.
- [ ] Buat detail kamar mandi.

## 🧱 Struktur

- [ ] Denah pondasi.
- [ ] Denah sloof.
- [ ] Denah kolom.
- [ ] Denah balok lantai 2.
- [ ] Denah pelat lantai 2.
- [ ] Detail tangga beton.
- [ ] Detail balok balkon.
- [ ] Cek torsi dan tulangan negatif balok balkon.
- [ ] Detail area basah lantai 2.
- [ ] Konfirmasi opsi struktur tumbuh 3 lantai.

## 🔌 MEP

- [ ] Denah air bersih.
- [ ] Denah air kotor dan air bekas.
- [ ] Denah vent pipe.
- [ ] Denah floor drain.
- [ ] Denah exhaust fan.
- [ ] Denah titik lampu.
- [ ] Denah saklar.
- [ ] Denah stopkontak.
- [ ] Denah AC dan drain AC.
- [ ] Sirkuit khusus ruang kerja/soldering.
- [ ] ELCB/RCD untuk area basah.

## 📐 CAD / Print

- [ ] Rapikan layer CAD.
- [ ] Rapikan CTB/lineweight untuk kertas putih.
- [ ] Buat layout A3 landscape.
- [ ] Plot test PDF A3.
- [ ] Tambahkan title block lengkap.
- [ ] Tambahkan skala dan tanggal revisi.
- [ ] Tambahkan nomor gambar.
